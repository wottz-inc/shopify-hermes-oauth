...
5. **Doctor audit health behavior** ([#37](https://github.com/wottz-inc/shopify-hermes-oauth/issues/37)): `doctor` verifies audit writability with a zero-byte append/open probe against the configured audit path. It may create the data directory or an empty audit file with owner-only permissions, but it must not append `doctor.audit_check` or any other health-check JSONL record to the main operational audit log.
