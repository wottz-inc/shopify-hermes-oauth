...
| #77 | Analytics, timeline, ShopifyQL evaluation | optional curated `shopify.analytics.shopifyql.summary`; timeline/events deferred | `read_reports`; customer events may need protected scopes | protected-data candidate | template-only, row/time-window limits, no arbitrary ShopifyQL | research note with implement/defer/out-of-scope decision |
...
| #90 | ShopifyQL gated reports | `shopify.analytics.shopifyql.summary` with allowlisted report IDs | `read_reports`; protected-data gate where needed | protected-data gated | template-only ShopifyQL, row/time-window limits | disabled by default; tests for missing scope, disabled gate, template allowlist |
