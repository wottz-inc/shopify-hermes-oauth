...
description: Shopify OAuth connector for Hermes: setup, reports, MCP, direct-token use.
version: 0.1.0
...
OAuth app installs, multi-store access, reports, MCP, durable workflows. Prefer direct-token `shopify` for one-off GraphQL.

## Safety rules

- Do not ask users to paste Shopify access tokens into chat.
- Do not ask users to paste Shopify client secrets into chat.
- Do not print OAuth secrets, access tokens, or token stores; keep MCP/CLI read-only unless requested.
...
Output: `Agent can do:` and `Human must do in Shopify:` without secrets.
...
Use Hermes Bitwarden Secrets Manager: `BWS_PROJECT_ID`, `--server-url`, `hermes secrets bitwarden status`, `hermes secrets bitwarden sync`. Do not write secrets back to `.env`.

Install: `npm pack && npm install -g ./wottz-shopify-hermes-oauth-*.tgz`. Hermes profile-local npm bin directories such as `$HERMES_HOME/node/bin` or `~/.hermes/node/bin` may be visible to Hermes but not to an ordinary SSH shell; use `export PATH="$HERMES_HOME/node/bin:$PATH"`. `Connector CLI: installed but not on PATH`.
...
If verification fails, report connector error; do not ask for raw tokens.
...
Prefer Markdown; use JSON for tooling; avoid unnecessary customer details.
...
`shopify.health` returns memory diagnostics. `mcp serve` emits lifecycle JSON to stderr. Webhook tools require `read_webhooks`; customer tools require `read_customers`; discount tools require `read_discounts`; marketing event tools require `read_marketing_events`; markets/locales require `read_markets` / `read_locales`; metaobject tools require `read_metaobject_definitions` / `read_metaobjects`. Custom data tools differ from standard reports: targeted metafield/metaobject schema questions with bounded value presence/length summaries. No raw GraphQL MCP or write/mutation custom data tool is exposed.
