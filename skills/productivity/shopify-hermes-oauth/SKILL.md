...
Configure Shopify with Application URL and `<public-https-url>/auth/callback`; install via `/auth/start?shop=<shop>.myshopify.com` while running.
...
`shopify.health` returns memory diagnostics without token-store contents. `mcp serve` emits lifecycle JSON to stderr.
...
If MCP is unavailable, fall back to CLI output without secrets.
