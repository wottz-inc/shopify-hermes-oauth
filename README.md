...
The MCP allowlist includes `shopify.b2b.companies.summary` and `shopify.b2b.catalogs.summary` for optional B2B visibility. Company summaries require `read_companies`; catalog/price-list summaries require `read_products` and catalog permissions where Shopify enforces them. See [`docs/b2b-summaries.md`](docs/b2b-summaries.md).

Both tools are bounded and read-only: companies are capped at 25, company locations at 10 per company, catalogs at 25, and price lists at 25. They return safe IDs/names/counts only and omit contacts, customer records, emails, phones, addresses, notes/tags, product lists, variant price exports, quantity-rule dumps, raw GraphQL input, and all B2B/catalog/price-list writes. Unsupported shops return structured `missing_scope`, `b2b_unavailable`, or `catalog_permission_required` statuses.
